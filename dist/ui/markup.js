...
export function renderCharacterStats(character) {
    return `
...
export function renderTraitList(traits) {
    return `
    <ul class="trait-list">
...
export function renderSelectedMark(isSelected) {
    if (!isSelected) {
        return "";
    }
    return `<span class="status-pill status-pill--selected" aria-label="Selected">✓</span>`;
}
export function renderConnectionDescription(description) {
...
export function renderExtraEffects(card) {
...
function renderCardArticle(card, controls, imageAltSuffix) {
...
function renderDeckSection(title, subtitle, cardsMarkup, tagMarkup = "", actionsMarkup = "") {
    return `
...
function renderDeckPanel(state) {
    if (!state.isDeckOpen) {
        return "";
    }
    ...
    const deckSection = renderDeckSection("My Deck", deckCapacityLabel, state.deck
        ...
        ? `
              ...
              `
        : "", "card art"))
        .join(""), `<p class="muted">Available card removals: ${availableCardRemovals}</p>`, "");
    return `
    <section class="deck-panel-wrap">
      <div class="deck-panel">
        ${bufferSection}
...
function renderDiscardPilePanel(state) {
    ...
    const canRetrieveFromDiscard = state.screen === "interview" &&
...
function renderSanityOverlay(state) {
