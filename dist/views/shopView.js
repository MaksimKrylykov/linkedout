...
function renderShopRows(state, run) {
    if (!state.shopSuggestions.length) {
        return "";
    }
    const renderConnectionTraits = (suggestion) => {
...
function renderPromoAside(state, playerName) {
    ...
    const promoCard = state.run.linkedOutTier === "none"
        ? `
...
function renderBoosterCount(label, rarity, count) {
    if (count === 0) {
        return "";
    }
...
function renderBrainCapacity(run, variant = "body") {
...
function renderLeekCodePremium(run) {
...
function renderLeekCodeSection(state, run) {
...
export function renderShopView(state) {
    ...
    const eligibleSuggestionCount = getEligibleSuggestionCount(state.data, state.connectedConnectionIds, state.retiredConnectionIds, state.run);
