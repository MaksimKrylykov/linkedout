...
function getInterviewerPhaseIndex(interviewer: Interviewer, phaseIndex: number): number {
...
export function getAwazonItemCost(run: Run, item: Item): number {
...
function getFreeItemBuys(connectedConnectionIds: ConnectionId[]): number {
...
function buildInterviewRetrySnapshot(
  state: AppState,
  run: Run,
  interviewer: Interviewer,
): InterviewRetrySnapshot {
...
export function canRetryInterviewRejection(state: AppState): boolean {
  if (
    state.screen !== "interview" ||
    !state.currentInterview ||
    !state.currentInterview.rejectionLetter ||
...
export function retryInterviewRejection(state: AppState): AppState {
...
export function appendPlayerChatMessage(state: AppState, message: string): AppState {
  if (
    !state.currentInterview ||
    state.screen !== "interview" ||
    state.isTurnResolving ||
