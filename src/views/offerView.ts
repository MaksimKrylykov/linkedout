...
import type { AppState, Card, Connection, InterviewRunSummary } from "../types.js";

function formatSalary(amount: number): string {
  return `$${amount.toLocaleString("en-US")}`;
}

function getOfferSalary(difficultyId: string): number {
...
function buildOfferLetter(characterName: string, difficultyName: string, salary: number): string[] {
  return [
    `Dear ${characterName},`,
...
function renderOfferDeckCard(card: Card): string {
  return `
...
function renderConnectionCards(connections: Connection[], retired = false): string {
    ...
    <div class="offer-results__interview-grid">
      ${history
        .map((entry) => {
          ...
          const statusLabel =
            entry.result === "on-time" ? "On Time" : entry.result === "overtime" ? "Overtime" : "DNF";
...
export function renderOfferView(state: AppState): string {
