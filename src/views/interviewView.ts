...
function getPhaseLabel(currentPhase: number): string {
  if (currentPhase === 0) {
    return "Main Problem";
  }
...
function renderInterviewResults(state: AppState): string {
  ...
  const renderBonusConnectionStrip = (connectionIds: string[]): string => {
            ...
            : `
              <div class="interview-slot interview-slot--filled${activeSlotIndex === index ? " interview-slot--active" : ""}">
                ${renderInterviewCard(
                  card,
                  "slot",
                  `data-action="remove-slotted-card" data-slot-index="${index}"`,
                  `Return ${card.name} to hand`,
                  isInteractionLocked,
                )}
              </div>
            `;
  ...
  const cardsMarkup = visibleCards
    .map((card, index) => {
      ...
      return renderInterviewCard(
        card,
        "hand",
        `data-action="play-hand-card" data-hand-index="${pageStart + index}"`,
        `Place ${card.name} in the next free slot`,
        isDisabled,
      );
    })
    .join("");
...
export function renderInterviewView(state: AppState): string {
  ...
  const currentPhaseDelay = getInterviewerDelay(interviewer, currentPhase);
          ...
          hasResolvedResults
            ...
            : `
              <section class="card interview-stage" aria-label="Interview arena">
                ...
                ${renderStageSlots(
                  state.currentInterview.slots,
                  slotEnergyRefills,
                  state.activeInterviewSlotIndex,
                  isInteractionLocked,
                )}
              </section>
            `
              ...
              <div class="interview-hand-card__header">
                ...
                <div class="interview-hand-card__status">
                  ...
                  >
                    <span>
                      Draw Card${
                        state.currentInterview.pendingDrawCount > 0
